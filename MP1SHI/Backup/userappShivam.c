...
void register_process(int pid) {
	char command[100];
	//FILE *fp;
	printf("I am in register_process\n");
	//fp = fopen("/proc/mp1/statusShiv","w");	
	/* Write to /proc/mp1/status */
        sprintf(command, "echo %d >/proc/mp1shi/statusSHI", pid);
...
void read_proc()
{
	...
	char command[] = "cat /proc/mp1shi/statusSHI";
	printf("Printing return value of read from /proc/mp1shi/statusShiv\n");
	printf("PID:CPU Time\n");
	system(command);
}
