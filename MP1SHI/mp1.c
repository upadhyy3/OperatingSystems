...
static struct Full_registration list_head;
 ...
 if ((buff = kcalloc(count + 1, sizeof(char), GFP_ATOMIC)) == NULL)
    ...
    if(!copy_from_user(buff, buffer, count)) 
    {
		...
		printk(KERN_INFO "Linked list executed");
            ...
            list_add_tail(&(full_registration->list), &(list_head.list));
            list_size++;
...
reset((void*) &buff);
   // kfree(buff);
   // *buff=NULL;
	printk(KERN_INFO "Value of linked list length:%d\n",list_size);
    return count;	
}
...
static void update_utime(struct work_struct * work) {
    ...
    mutex_lock(&list_lock);
    list_for_each_entry_safe(curr_proc, tmp, &list_head.list, list) {
	printk(KERN_INFO "We are in the workQueue and CPU Time is %lu",curr_proc->cpu_time);
...
static void sched_work_function(unsigned long data) {
	...
	printk(KERN_INFO "I am in the Sched Work Function\n");
