 ...
 *
 */
int mp2_read(struct file *file,const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0, i=1;
...
void mp2_register_process(char *user_data)
{
	...
	new_task = kmalloc(sizeof(*new_task), GFP_ATOMIC);
 ...
 *
 */
int mp2_write(struct file *filp, const char __user *buff,
...
static int __init mp2_init_module(void)
{
	...
	proc_dir = proc_mkdir("mp2psh", NULL);
	...
	else 
	{
		...
		mp2_current = NULL;
                ...
                mp2_sched_kthread = kthread_run(mp2_sched_kthread_fn,NULL, "mp2_sched_kthread");
