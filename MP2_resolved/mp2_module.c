...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Implementing rate monitoring system");
MODULE_AUTHOR("Piyush, Shivam and Xiaoming");
...
{
	printk(KERN_INFO "\nG22:MP2 - Inside Admission control\n");
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering select_process_from_lst\n");
	...
	mutex_lock(&process_mutex);
	printk(KERN_INFO "\nG22:MP2 - Searching in the list for for the READY task with the highest priority (shortest period).\n");
	list_for_each_entry_safe(curr, next, &process_head, list)
	{
	...
	if(high)  /*found the ready job with highest priority(compared to other ready jobs)*/
	{
		...
		{
			printk(KERN_INFO "G22:MP2 - Task %d has the highest priority among those ready but lower than the currently running task" 
							"(pid: %d)--no preemption", high->pid, curr_running_task->pid);
		}
		else /*If no, the the selected job is ready to preempt other jobs */
		{
			printk(KERN_INFO "G22:MP2 - Task %d has the highest priority and will preempt the current running task"
										"(if any)\n", high->pid);
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering add_process_to_list\n");
	list_add_tail(&task->list, &process_head); // behaves like a FIFO queue
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering clean_up_list\n");
	mp2_task_obj *curr, *next;
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering get_user_info\n");
	mp2_task_obj *curr, *next;
	int bytes=0;
	list_for_each_entry_safe(curr, next, &process_head, list)
	{
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering wakeup_timer_callback()\n");
	unsigned long flags;
...
//static int mp2_read(char* buffer, char** buffer_location, off_t offset, int buffer_length, int* eof, void* data)
static ssize_t mp2_read(struct file *file,const char __user *buffer, size_t count, loff_t *data)
{
	printk(KERN_INFO "\nG22:MP2 - Entering read_proc_cb\n");
	int bytes=0;
	char* temp;
	...
	/*if(offset>0)
	{
		printk(KERN_INFO "G22:MP2 - read proc - offset is grtr than 0. Offset =  %d\n", offset);
		return bytes;
	}
	*/
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering get_info_from_msg()\n");
...
{
	printk(KERN_INFO "\nG22:MP2 - Entering yield_process fucntiom\n");
	mp2_task_obj *curr, *next;
	long int dt;
	long int offset;
	int ret;
...
static int mp2_write(struct file* file, const char __user*  buffer, unsigned long count, void* data)
{
	printk(KERN_INFO "\nG22:MP2 - Begin write process\n");
...
{
	printk(KERN_INFO "\nG22:MP2 - mp2_module initialization\n");
